...
function Fields({ partner, errors = {} }: { partner?: PartnerValues; errors?: Record<string, string[]> }) {
  ...
  return (
    <>
      <div className="grid gap-4 md:grid-cols-2">
        <FieldRow label="Name">
          <Input name="name" required defaultValue={partner?.name ?? ""} maxLength={140} aria-invalid={Boolean(errors.name)} placeholder="Vodafone" />
      ...
      <FieldRow label="Caption (anchor only — shows under the name)">
        <Textarea name="caption" rows={2} defaultValue={partner?.caption ?? ""} maxLength={280} aria-invalid={Boolean(errors.caption)} placeholder="Telecom · enterprise growth · multi-year retainer" />
        <FieldError messages={errors.caption} />
      </FieldRow>

      <div className="grid gap-4 md:grid-cols-2">
        <FieldRow label="Upload logo">
          <Input name="logo_file" type="file" accept="image/svg+xml,image/png,image/jpeg,image/webp" />
          <p className="mt-1 text-[12px] text-[var(--admin-muted)]">
        ...
        <FieldRow label="Logo URL (if not uploading)">
          <Input name="logo_url" type="url" defaultValue={partner?.logo_url ?? ""} placeholder="/partners/vodafone.png or https://..." aria-invalid={Boolean(errors.logo_url)} />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <FieldRow label="Sort order">
          <Input name="sort_order" type="number" defaultValue={sortOrder} aria-invalid={Boolean(errors.sort_order)} placeholder="0" />
          <FieldError messages={errors.sort_order} />
        </FieldRow>

        <FieldRow label="Visibility">
          <label className="flex items-center gap-2 text-[14px]">
            <input type="checkbox" name="is_active" defaultChecked={isActive} className="h-4 w-4 accent-[var(--admin-accent)]" />
...
function CreateButton() {
  ...
  return <Button type="submit" disabled={pending}>{pending ? "Saving..." : "Create partner"}</Button>;
}

function PartnerCreateInner({ partner }: { partner?: PartnerValues }) {
  const [state, formAction] = useFormState(createClientPartnerAction, initial);
  ...
  return (
    <form action={formAction} encType="multipart/form-data" className="flex max-w-[900px] flex-col gap-6 border border-[var(--admin-border)] bg-[var(--admin-panel)] p-6">
      <Fields partner={partner} errors={errors} />
...
function PartnerEditInner({ partner }: { partner?: PartnerValues }) {
  const { formRef, status, errorMsg, onFormChange } = useAutoSave(updateClientPartnerAction, initial);
...
export default function PartnerForm({ mode, partner }: { mode: "create" | "edit"; partner?: PartnerValues }) {
  if (mode === "create") return <PartnerCreateInner partner={partner} />;
  return <PartnerEditInner partner={partner} />;
}
